...
def plants_list():
    ...
    print('printing plants', plants_data)
...
def create():
    ...
    if request.method == 'POST':
        ...
        inserted_plant = result = mongo.db.plants.insert_one(new_plant)
...
def detail(plant_id):
    ...
    # collection.
    all_harvests = mongo.db.harvests.find()
    harvests_for_plant = mongo.db.harvests.find({'plant_id': plant_id})
    plant_harvests = []
    
    for harvest in harvests_for_plant:
        plant_harvests.append(harvest)

    print(harvests_for_plant)
    context = {
        'plant' : plant_to_show,
...
def edit(plant_id):
    ...
    if request.method == 'POST':
        ...
        changes = { '$set': {'name': name, 'variety': variety, 'photo_url': photo_url, 'date_planted': date_planted}}

        mongo.db.plants.update_one(search, changes)
